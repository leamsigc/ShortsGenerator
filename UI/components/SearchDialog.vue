...
const searchGroups = ref<SearchGroups>([
  ...
  },
  {
    ...
    items: [
      ...
      {
        iconName: themeToggleIcon.value,
        iconImage: null,
        key: 7,
        title: "Theme toggle",
        label: t("searchDialog.actionsOptions.themeToggle"),
        action() {
          toggleColorMode();
        },
      },
  ...
  const newGroups: SearchGroups = searchGroups.value
    .map((group) => {
      const items = group.items.filter((item) => {
        const titleMatch = keywords.value.some((k) =>
          item.title.toLowerCase().includes(k.toLowerCase())
        );
        const tagsMatch =
          item.tags &&
          keywords.value.some((k) =>
            item.tags?.toLowerCase().includes(k.toLowerCase())
          );
...
function performAction() {
  const item = filteredFlattenItems.value.find(
    (item) => item.key === activeItem.value
  );
  if (item) {
